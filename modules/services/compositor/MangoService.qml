...
Item {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (kbMatch) {
          ...
          if (kbName !== internal.currentKbLayout) {
            ...
            if (typeof KeyboardLayoutService !== 'undefined') {
    ...
    function updateWindows() {
      ...
      for (let i = 0; i < toplevels.length; i++) {
        ...
        if (outputIdx === undefined) {
          console.error("[MangoService] ERROR: No output index for", outputName, "- this should not happen!");
  ...
  function initialize() {
    ...
    console.log("[MangoService] Initializing MangoWC/DWL compositor integration");
  ...
  function cycleKeyboardLayout() {
    console.warn("[MangoService] Keyboard layout cycling not supported");
  }
  ...
  function spawn(command) {
    try {
      ...
      console.error("[MangoService] Failed to spawn command:", e);
    }
  }
}
