...
Item {
  ...
  Process {
    ...
    onExited: function (exitCode) {
      if (exitCode !== 0 || !accumulatedOutput) {
        console.error("[SwayService]", "Failed to query tree, exit code:", exitCode);
        accumulatedOutput = "";
        return;
      }
      ...
      try {
        ...
        console.error("[SwayService]", "Failed to parse tree:", e);
      } finally {
        accumulatedOutput = "";
      }
    }
  ...
  function handleInputEvent(ev) {
    try {
      ...
      console.error("[SwayService]", "Error handling activelayout:", e);
    }
  }
  ...
  function cycleKeyboardLayout() {
    try {
      ...
      console.error("[SwayService]", "Failed to cycle keyboard layout:", e);
    }
  }
  ...
  function spawn(command) {
    try {
      ...
      console.error("[SwayService]", "Failed to spawn command:", e);
    }
  }
}
