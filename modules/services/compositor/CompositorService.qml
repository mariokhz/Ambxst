...
Singleton {
  ...
  // Backward-compatible flat arrays/maps (mirrors HyprlandData API)
  ...
  Component.onCompleted: {
    // Load display scales from StateService
    Qt.callLater(() => {
                   if (typeof StateService !== 'undefined' && StateService.initialized) {
  ...
  Connections {
    target: typeof StateService !== 'undefined' ? StateService : null
  ...
  function loadDisplayScalesFromState() {
    try {
      const cached = StateService.get("displayScales", null);
      ...
      console.warn("[CompositorService] Failed to load display scales:", error);
      displayScalesLoaded = true;
    }
  }
  ...
  function updateActiveWorkspaceId() {
  ...
  function syncWindows() {
    ...
    for (var i = 0; i < ws.length; i++) {
      windows.append(ws[i]);
      newWindowList.push(ws[i]);
      const wsId = ws[i].workspaceId;
      if (wsId !== undefined && wsId !== null) {
        occupationMap[wsId] = true;
        if (!windowsMap[wsId]) windowsMap[wsId] = [];
        windowsMap[wsId].push(ws[i]);
      }
    }
    // Update flat arrays for backward compatibility with HyprlandData consumers
  ...
  function updateDisplayScales() {
    if (!backend || !backend.queryDisplayScales) {
      console.warn("[CompositorService] Backend does not support display scale queries");
      return;
    }
  ...
  function saveDisplayScalesToCache() {
    try {
      if (typeof StateService !== 'undefined' && StateService.initialized) {
        StateService.set("displayScales", displayScales);
      }
    } catch (error) {
      console.warn("[CompositorService] Failed to save display scales:", error);
    }
  }
  ...
  function switchToWorkspace(workspace) {
    if (backend && backend.switchToWorkspace) {
      ...
      console.warn("[Compositor] No backend available for workspace switching");
    }
  }

  // Relative workspace switching (for scroll wheel: delta = +1 or -1)
  function switchRelativeWorkspace(delta) {
    // Find current workspace and navigate relative to it
    const current = getCurrentWorkspace();
    if (!current) return;

    // Find current workspace position in the model
    let currentIdx = -1;
    for (let i = 0; i < workspaces.count; i++) {
      if (workspaces.get(i).id === current.id) {
        currentIdx = i;
        break;
      }
    }
    ...
    if (targetIdx >= 0 && targetIdx < workspaces.count) {
      const target = workspaces.get(targetIdx);
      switchToWorkspace({id: target.id, idx: target.idx, output: target.output});
    }
  }
  ...
  function focusWindow(window) {
    if (backend && backend.focusWindow) {
      ...
      console.warn("[Compositor] No backend available for window focus");
    }
  }
  ...
  function closeWindow(window) {
    if (backend && backend.closeWindow) {
      ...
      console.warn("[Compositor] No backend available for window closing");
    }
  }
  ...
  function spawn(command) {
    ...
    if (backend && backend.spawn) {
      ...
      try {
        ...
        console.error("[CompositorService] Failed to execute detached:", e);
      }
    }
  }
  ...
  function getCustomCommand(action) {
    try {
      const powerOptions = Config.sessionMenu?.powerOptions || [];
  ...
  function executeSessionAction(action, defaultCommand) {
    ...
    if (customCommand) {
      console.log("[Compositor]", `Executing custom command for action: ${action} Command: ${customCommand}`);
  ...
  function logout() {
    ...
    if (backend && backend.logout) {
      ...
      console.warn("[Compositor] No backend available for logout");
    }
  }
  ...
  function lockAndSuspend() {
    ...
    // Lock the screen first, then suspend after a short delay
    GlobalStates.lockscreenVisible = true;
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (GlobalStates.lockscreenVisible) {
        ...
        // Max 3 seconds wait
        console.warn("[Compositor] Lock screen failed to activate, suspending anyway");
        stop();
        lockAndSuspendCheckCount = 0;
        suspend();
      }
