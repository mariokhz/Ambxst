...
Item {
      ...
      try {
        ...
        for (const keyboard of devicesData.keyboards) {
          if (keyboard.main) {
            ...
            if (typeof KeyboardLayoutService !== 'undefined') KeyboardLayoutService.setCurrentLayout(layoutName);
  ...
  function extractWindowData(toplevel) {
    ...
    try {
      ...
      const data = {
        ...
        "isFocused": focused,
        "output": output,
        // ─── Legacy HyprlandData-compatible fields ───────────────────────
        // These are used by Overview / ScrollingWorkspace / OverviewWindow
        "address": windowId,
        "class": appId,
        "workspace": { "id": wsId || -1 },
        "monitor": toplevel.monitor?.id ?? -1,
        "at": [0, 0],
        "size": [100, 100],
        "floating": false,
        "fullscreen": false,
      ...
      // Enrich with raw IPC data when available (Hyprland only)
      try {
        const ipc = toplevel.lastIpcObject;
        if (ipc) {
          if (Array.isArray(ipc.at))         data.at = ipc.at;
          if (Array.isArray(ipc.size))        data.size = ipc.size;
          if (typeof ipc.floating === "boolean") data.floating = ipc.floating;
          if (typeof ipc.fullscreen === "boolean") data.fullscreen = ipc.fullscreen;
          if (typeof ipc.xwayland === "boolean")   data.xwayland = ipc.xwayland;
          if (typeof ipc.focusHistoryID === "number") data.focusHistoryID = ipc.focusHistoryID;
          if (typeof ipc.monitor === "number") data.monitor = ipc.monitor;
        }
      } catch (e) { /* lastIpcObject not available on this compositor */ }
  ...
  function focusWindow(window) {
    try {
      if (!window || !window.id) {
        console.warn("[HyprlandService]", "Invalid window object for focus");
        return;
      }
  ...
  function cycleKeyboardLayout() {
    try {
      ...
      console.error("[HyprlandService]", "Failed to cycle keyboard layout:", e);
    }
  }
  ...
  function spawn(command) {
    try {
      ...
      console.error("[HyprlandService]", "Failed to spawn command:", e);
    }
  }
}
