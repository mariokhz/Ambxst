...
# Get list of available mime types once
if ! AVAILABLE_TYPES=$(wl-paste --list-types 2>/dev/null); then
    exit 0
fi

# Check for files first (text/uri-list)
if echo "$AVAILABLE_TYPES" | grep -q "^text/uri-list$"; then
...
# Check for images
if IMAGE_MIME=$(echo "$AVAILABLE_TYPES" | grep '^image/' | head -1); then
...
if echo "$AVAILABLE_TYPES" | grep -q "text/plain"; then
    # Try with charset first
